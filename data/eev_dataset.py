...
'''
注意：load_label函数中可能需要从toy数据集调整回实际的数据集
'''
...
class EEVDataset(BaseDataset):
    ...
    def load_label(self):
        ...
        for _id in tqdm(self.video_ids, desc=self.set_name):#
            ...
            if self.set_name != 'test':
                ...
                self.target[_id]['label'] = torch.from_numpy(label_h5f[_id]['label'][()]).float()
            else:
                self.target[_id]['timestamp'] = torch.from_numpy(label_h5f[_id]['timestamp'][()]).long()
                self.target[_id]['length'] = torch.as_tensor(len(self.target[_id]['timestamp'])).long()#
    ...
    def collate_fn(self, batch):
        ...
        if self.set_name != 'test':
            ...
            label = pad_sequence([sample['label'] for sample in batch], padding_value=torch.tensor(0.0), batch_first=True)
        ...
        mask = torch.zeros([batch_size, batch_max_length, 15]).float() #15个类别
